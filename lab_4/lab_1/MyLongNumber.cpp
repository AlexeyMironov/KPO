...
const CMyLongNumber operator / (const CMyLongNumber &num1, const CMyLongNumber &num2)
{
	...
	/*for (int i = 0; i < result.size(); i++)
	{
		if (result[i] > 9)
		{
			string number = to_string(result[i]);
			for (int j = 0; j < number.size(); j++)
			{
				res.push_back(static_cast<int>(number[j]) - 48);
			}
		}
		else
		{
...
const CMyLongNumber sqrt(const CMyLongNumber &num)
{
	...
	one.push_back(1);
	CMyLongNumber res;
	while (GetLargestNumber(l.GetNumber(), r.GetNumber()) == 2 || GetLargestNumber(l.GetNumber(), r.GetNumber()) == 3)
	{
		CMyLongNumber m = (l + r) / two;
		CMyLongNumber mm = m*m;
		if ((GetLargestNumber(mm.GetNumber(), num.GetNumber()) == 2 || GetLargestNumber(mm.GetNumber(), num.GetNumber()) == 3))
		{
			res = m;
			l = m + one;
		}
